	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			try {
				...
				String sparqlQueryString = "select distinct ?Entity (<http://www.w3.org/2000/01/rdf-schema#label> AS ?rel) ?EntityLabel (<http://dummy.edu/graph> AS ?graph) where { \n"
						+ "{graph ?g1 { \n"
						+ "?Entity <http://www.w3.org/2004/02/skos/core#altLabel> ?EntityLabel . \n"
						+ "}\n"
						+ "} UNION {\n"
						+ "graph ?g2 { \n"
						+ "?Entity <http://www.w3.org/2004/02/skos/core#prefLabel> ?EntityLabel . \n"
						+ "}\n"
						+ "} UNION {\n"
						+ "graph ?g3 { \n"
...
//						+ " ORDER BY DESC (?Entity)";
				...
				while(results.hasNext()) {
					try {
						...
						String relation = qs.get("rel").toString() + line_number;
