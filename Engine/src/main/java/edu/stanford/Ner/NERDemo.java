...
public class NERDemo {
	...
	public static void main(String[] args) throws Exception {
		...
		String serializedClassifier = "StanfordNerClassifiers/english.all.3class.distsim.crf.ser.gz";
		...
		if (args.length > 1) {
			...
			DocumentReaderAndWriter<CoreLabel> readerAndWriter = classifier
					.makePlainTextReaderAndWriter();
			classifier.classifyAndWriteAnswersKBest(args[1], 10,
					readerAndWriter);
			...
			for (String str : example) {
				System.out
						.print(classifier.classifyToString(str, "tsv", false));
			}
			System.out.println("---");
			...
			for (String str : example) {
				...
				for (Triple<String, Integer, Integer> trip : triples) {
					System.out
							.printf("%s over character offsets [%d, %d) in sentence %d.%n",
